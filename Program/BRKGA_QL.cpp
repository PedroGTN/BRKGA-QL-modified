...
void saveQtable(std::string name){
    std::ofstream qtable(name);

    for(auto row : Q){
        for(auto col : row){
            qtable<<col.S<<':'<<col.pVar<<':'<<col.q<<':'<<col.k<<':'<<col.kImp<<'|';
        }
        qtable<<"\n";
    }
}
...
void loadQtable(std::string name){
    ...
    for (unsigned int j=0; j<sizeof(Rhoe)/sizeof(Rhoe[0]); j++)
    {
        char dud;
        TQ aux;
        qtable>>aux.S>>dud>>aux.pVar>>dud>>aux.q>>dud>>aux.k>>dud>>aux.kImp>>dud;
        std::cout<<aux.S<<" "<<aux.pVar<<" "<<aux.q<<" "<<aux.k<<" "<<aux.kImp<<" | ";
