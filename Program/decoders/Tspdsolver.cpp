...
void Tspd_solver::print_fstsp_tour(Tspd_problem &instance, vector<uint64_t> &permutation){
    fstsp_tour final_tour;
    Digrafo graph(instance.getN());
    vector<int> permutation_int(permutation.begin(), permutation.end()), predecessor;

    split_lazy(instance, permutation_int, predecessor, graph);
    ...
    for(int j=predecessor[ant]; j>=0; j = predecessor[j]){
        ...
        while(a!=nullptr){
            ...
            final_tour.push_back(make_pair(min_arc, j));
            ant = j;
    }

    for(int64_t j=(int64_t)final_tour.size()-1; j>=0; j--){
        cout<<final_tour[j].second<<","<<final_tour[j].first->drone_node<<","<<(final_tour[j].first->dest==instance.getN()?0:final_tour[j].first->dest)<<"|";
    }
